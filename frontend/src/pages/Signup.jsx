...
const Signup = () => {
  ...
  // If already logged in, send directly to onboarding/dashboard
  useEffect(() => {
    if (isAuthenticated) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-12 dark:bg-darkbg sm:px-6 lg:px-8">
      <div className="absolute top-0 right-1/4 h-[300px] w-[300px] rounded-full bg-indigo-400/10 blur-3xl dark:bg-indigo-600/5"></div>
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-white p-8 shadow-xl border border-slate-100 dark:bg-slate-900 dark:border-darkbg-border animate-slide-in">
        ...
        <div className="text-center">
          ...
          <h2 className="mt-4 text-3xl font-extrabold text-slate-900 dark:text-white">Create Account</h2>
          <p className="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
            Start generating tailored AI learning timelines
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-600 dark:text-slate-400" htmlFor="name">
              Username
            </label>
        ...
        <div className="text-center pt-2">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            ...
            <Link to="/login" className="font-semibold text-accent hover:text-accent-dark dark:text-blue-400">
              Sign in
            </Link>
          </p>
        </div>
